...
function App() {
  ...
  const PostFetchData = async (filteredText) => {
    ...
    myHeaders.append(
      "Authorization",
      "Bearer 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"
    );
  ...
  return (
    <div className="App">
      ...
      <header className="App-header">
        ...
        {
          selectedFile === null ? ("") : (  <div style={{ padding: "15px 10px" }}>
          {resultData.length === 0 ? (
            <table
              style={{
                width: "100%",
          ...
          <tr>
            {headers.map((header) => (
              <th style={{ border: "2px solid black", padding: "8px" }} key={header}>{header}</th>
            ))}
          </tr>
        </thead>
  ...
  {resultData.map((row, rowIndex) => (
    <tr key={rowIndex}>
      {headers.map((header, columnIndex) => (
        <td key={`${rowIndex}-${columnIndex}`} style={{ border: "2px solid black", padding: "8px" }}>
          {row[header]}
        </td>
      ))}
    </tr>
